...
export async function fixWorkflowIdCommand(nodeOrUri?: any, nodesOrUris?: any[]) {
	// Support multi-selection
	let urisToProcess: vscode.Uri[] = [];

	if (nodesOrUris && Array.isArray(nodesOrUris) && nodesOrUris.length > 0) {
		urisToProcess = nodesOrUris.map(n => n.fsPath ? vscode.Uri.file(n.fsPath) : n);
	...
	for (const uri of urisToProcess) {
		try {
			...
			if (!isValidNanoId(workflowData.id)) {
				...
				fs.writeFileSync(targetPath, JSON.stringify(workflowData, null, 4), "utf-8");
				fixedCount++;
			} else {
				skippedCount++;
			}
	...
	if (fixedCount > 0) {
		vscode.window.showInformationMessage(`[Automa Auto-Fix] Đã tạo ID mới cho ${fixedCount} file(s).`);
	} else if (skippedCount > 0) {
		vscode.window.showInformationMessage(`[Automa Auto-Fix] ${skippedCount} file(s) đã có ID hợp lệ, không cần sửa.`);
	} else if (errorCount > 0) {
		vscode.window.showErrorMessage(`[Automa Auto-Fix] Lỗi khi xử lý ${errorCount} file(s). Kiểm tra console.`);
	}
}
