...
export class WorkflowPreviewEditorProvider implements vscode.CustomTextEditorProvider {
	...
	public async resolveCustomTextEditor(
		...
		const updateWebview = () => {
			try {
				...
				webviewPanel.webview.html = this.getHtmlContent(json, triggerParams, updatedAt);
			} catch (e: any) {
	...
	private getHtmlContent(json: any, triggerParams: any[], updatedAt: number): string {
	...
	<body class="flex h-screen flex-col bg-vsc-bg text-vsc-fg font-sans m-0 overflow-hidden">
		...
		<div class="flex border-b border-vsc-border px-6 mt-2">
			...
			<button id="tabBtn-data" class="px-4 py-2 border-b-2 border-transparent text-vsc-muted hover:text-vsc-fg font-medium" onclick="switchTab('data')">Settings & Data</button>
		...
		<div class="flex-1 overflow-y-auto relative p-6">
			...
			<form id="workflowForm">
				...
				<!-- DATA TAB -->
				<div id="tab-data" class="hidden max-w-3xl">
					<div class="flex flex-col gap-4">
						<div class="flex flex-col gap-1">
		...
		<script>
			...
			const triggerParams = ${paramsJson};
	
			// 1. Tab Switching Logic
			function switchTab(tabId) {
				const tabs = ['params', 'data', 'properties'];
				tabs.forEach(id => {
			...
			const currentIcon = "${json.icon || 'riGlobalLine'}";
			
			if (currentIcon && !iconsList.includes(currentIcon)) {
				iconsList.push(currentIcon);
			}
			...
			iconsList.forEach(iconName => {
				...
				if (iconName === currentIcon) option.selected = true;
			...
			// 3. Render Trigger Parameters Form
			...
			if (triggerParams.length > 0) {
				...
				triggerParams.forEach(param => {
					formHtml += '<div class="flex flex-col gap-1">' +
						...
						'<input type="text" name="' + param.name + '" value="' + (param.defaultValue || '') + '" placeholder="' + (param.description || '') + '" class="w-full px-3 py-2 bg-vsc-input text-vsc-inputText border border-vsc-inputBorder rounded focus:outline-none focus:border-vsc-button">' +
			...
			// 5. Handle Open in Studio Button
