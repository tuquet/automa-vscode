...
export class AutomaFilesProvider implements vscode.TreeDataProvider<FileItem> {
    ...
    setSearchQuery(query: string): void {
        ...
        vscode.commands.executeCommand('setContext', `${this.viewId}.isFiltered`, this.searchQuery !== '');
        this.refresh();
    }
    ...
    async getChildren(element?: FileItem): Promise<FileItem[]> {
        ...
        let files = await vscode.workspace.findFiles(this.globPattern, '**/node_modules/**');
        ...
        if (this.searchQuery) {
            result = result.filter(item => item.label.toLowerCase().includes(this.searchQuery));
        }
...
class FileItem extends vscode.TreeItem {
    constructor(
        ...
        this.description = vscode.workspace.asRelativePath(path.dirname(this.resourceUri.fsPath));
